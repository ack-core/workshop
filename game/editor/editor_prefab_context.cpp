...
namespace game {
    ...
    void EditorNodePrefab::setResourcePath(const API &api, const std::string &path) {
        if (path.empty()) {
            ...
            worldObject->loadResources([&api](core::WorldInterface::Object &obj) {
    ...
    EditorPrefabContext::EditorPrefabContext(API &&api, NodeAccessInterface &nodeAccess) : _api(std::move(api)), _nodeAccess(nodeAccess) {
        EditorNode::makeByType[std::size_t(core::WorldInterface::NodeType::PREFAB)] = [](std::size_t typeIndex) {
            ...
            for (auto &index : descs) {
                ...
                if (index.first.find('.') == std::string::npos) { // root node comes first
                    if (_nodeAccess.hasNodeWithName(index.first)) {
                        _api.platform->sendEditorMsg("engine.failmsg", "Attempt to edit a prefab with the root node '" + index.first + "'. But there is already a node with that name.");
                        break;
                    }
                    position = node->localPosition; // place tree where the prefab is
                }

                const core::WorldInterface::NodeType type = static_cast<core::WorldInterface::NodeType>(index.second->getInteger("type", 0));
                const std::string resourcePath = index.second->getString("resourcePath", EDITOR_EMPTY_RESOURCE_PATH);
    ...
    bool EditorPrefabContext::_savePrefab(const std::string &data) {
        struct fn {
            static void addNode(util::Description &out, EditorNode &node, bool isRoot) {
                util::Description *sub = out.setDescription(node.name.data());
                const math::vector3f position = isRoot ? math::vector3f(0, 0, 0) : node.localPosition;
                sub->setInteger("type", std::int64_t(node.type));
                sub->setVector3f("position", position);
                sub->setString("resourcePath", node.resourcePath);
        ...
        if (std::shared_ptr<EditorNode> node = _nodeAccess.getSelectedNode().lock()) {
            if (node->type == core::WorldInterface::NodeType::PREFAB) {
                _api.platform->sendEditorMsg("engine.failmsg", "Cannot save prefab as prefab");
                return true;
            }
            if (node->parent != nullptr) {
                _api.platform->sendEditorMsg("engine.failmsg", "Only top-level nodes can be saved as prefab");
                return true;
            }
            ...
            if (_validatePrefab(*node) == false) { // no errors
                ...
                _api.platform->sendEditorMsg("engine.failmsg", "Subtree is not valid for saving");
            }
        }
