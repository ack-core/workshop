...
namespace game {
    ...
    void EditorNodeGround::setResourcePath(const API &api, const std::string &path) {
        if (path.empty()) {
            ...
            resourcePath = path;
            api.resources->getOrLoadGround(path.c_str(), [this, &api](const foundation::RenderDataPtr &msh, const foundation::RenderTexturePtr &t, const resource::GroundMapDescription &desc) {
